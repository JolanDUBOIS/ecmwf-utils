...
class StorageManager:
    ...
    def allocate(self, meta: RetrievalMeta) -> RetrievalTicket:
        """ Allocate storage for a new retrieval based on its metadata. """
        subfolder = self.base_folder / meta.level
        data_subfolder = subfolder / "data"
        queries_subfolder = subfolder / "queries"
        ...
        query_file_path = queries_subfolder / f"query_{meta.id}_{now_timestamp}.json"
        ...
        if query_file_path.exists():
            logger.error(f"File {query_file_path} already exists. Allocation failed.")
            raise FileExistsError(f"File {query_file_path} already exists.")
    ...
    def _add_index_entry(self, query: Query, ticket: RetrievalTicket) -> None:
        ...
        entry = {
            "data_file": str(ticket.data_file_path),
            "query_file": str(ticket.query_file_path),
